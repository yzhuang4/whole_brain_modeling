...
num_iterations = 500;
filelist = dir('Estimated_Parameter_destrieux_hc_500*.mat');
num_files = length(filelist);

hc.Para_E = zeros([298, num_files]);
...
% load hiv baseline
filelist = dir('Estimated_Parameter_destrieux_hiv_500*.mat');
num_files = length(filelist);

hiv.Para_E = zeros([298, num_files]);
...
filelist = dir('Estimated_Parameter_destrieux_hiv_12wk_500*.mat');
num_files = length(filelist);

wk12.Para_E = zeros([298, num_files]);
...
saveas(figure(1), [figure_dir, '/agematch_relax-mfm_result_500iters_rmltnp_destrieux.fig'])
saveas(figure(1), [figure_dir, '/agematch_relax-mfm_result_500iters_rmltnp_destrieux.pdf'])
...
legend([f1 f2 f3], 'hc','hiv bsl','hiv 12wk', 'Location', 'southeast')
hold off
%saveas(figure(2), [figure_dir, '/agematch_relax-mfm_result_500iters2_rmltnp_destrieux.fig'])
%saveas(figure(2), [figure_dir, '/agematch_relax-mfm_result_500iters2_rmltnp_destrieux.pdf'])
...
xlabel('group')
hold off
%saveas(figure(3), [figure_dir, '/agematch_relax-mfm_result_500iters3_rmltnp_destrieux.fig'])
%saveas(figure(3), [figure_dir, '/agematch_relax-mfm_result_500iters3_rmltnp_destrieux.pdf'])
...
save('500iters_final_estimate_Paras_agematched_1024_destrieux.mat','hc','hiv','wk12');
