...
%parameter_file_name = 'Estimated_Parameter_hc_500_2019413.mat';
%parameter_file_name = '500iters_final_estimate_Paras_agematched_0708.mat';
parameter_file_name = '500iters_final_estimate_Paras_agematched_1024_destrieux.mat';

%parameter_file_name = 'Estimated_Parameter_hc_2019411.mat';
load ([data_dir '/' parameter_file_name],'hc');

Para_E = hc.Para_E_avg;
...
load(fullfile(data_dir,'train_test_dataset_agematch_destrieux_1004.mat'))
FC = hc_fc_test;
FC = FC ./ max(max(FC));
SC = hc_sc_test; 
SC = SC./max(max(SC)).*0.2;
...
funcP = @(Para_E,Nstate) CBIG_MFMem_rfMRI_nsolver_eul_sto_resLH(Para_E,SC,y,FC_mask,Nstate,7,2,0);
...
save( [save_dir '/agematched_1000simulation_testGrp_resLow_hc_' num2str(saved_date(1)) num2str(saved_date(2)) num2str(saved_date(3))],'CC_check','h_output','BOLD_d');
rmpath(lib_dir);
